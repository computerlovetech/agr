...
- Adding a new tool (e.g., `agr config add tools cursor`) no longer fails with a spurious "not a valid skill (missing SKILL.md)" error when ralph dependencies exist in `agr.toml`. Ralphs are now correctly skipped during tool-sync.
- Skills whose directory name matched an excluded name (`build`, `dist`, `vendor`, `venv`, etc.) were silently invisible during discovery — even when the excluded name was the skill directory itself, not an ancestor. For example, a skill at `skills/build/SKILL.md` would not be found. Fixed to only exclude ancestor directories.
- Filenames containing `..` (e.g., `my..config.txt`) were incorrectly rejected by the path traversal check. Only actual `..` directory traversal segments are now blocked.
- The `canonical_instructions` example generated by [`agr init`](reference.md#agr-init) showed `"claude"` (a tool name), but validation requires an instruction filename like `"CLAUDE.md"`. Users who uncommented the example would get a `ConfigError`.
- `agr add -g ./local-skill` wrote a different path format to `agr.lock` than to `agr.toml`, causing [`agr sync --locked`](reference.md#agr-sync) to incorrectly report the lockfile as stale after a global local install.
- Long paths in CLI error and detail messages are no longer word-wrapped by the terminal, making output easier to read and parse.
